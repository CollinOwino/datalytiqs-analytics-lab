-- Certification governance hardening for the MEAL release.
-- The database is the final issuance boundary: only an approved, attributable human review may issue a credential.
...
begin
 ...
 if v_request.status <> 'approved' then raise exception 'Credential review must be approved before issuance'; end if;
 if v_request.reviewed_by is null or v_request.reviewed_at is null then raise exception 'Approved review must include reviewer identity and timestamp'; end if;
