...
export default function MerlFoundations(){
 ...
 return <main className="merl-main" style={{maxWidth:1100,margin:'0 auto',padding:'40px 24px',fontFamily:'system-ui',color:'#0b2c4d'}}>
  ...
  <section style={{display:'grid',gap:14}}>{modules.map((m,index)=>{const locked=index>0&&!complete(modules[index-1].id),expanded=open===m.id,mp=p(m.id),isComplete=complete(m.id);return <article key={m.id} style={{border:'1px solid #d7dee7',borderRadius:12,overflow:'hidden',opacity:locked?.58:1}}>
   ...
   {expanded&&!locked&&<div id={`module-panel-${m.id}`} className="module-panel" style={{padding:'0 24px 24px'}}><hr style={{border:0,borderTop:'1px solid #e5e7eb'}}/><h3>Learning sequence</h3>{m.lessons.map((lesson,i)=>{const k=`${m.id}-${i}`,done=!!mp?.lesson_evidence?.[k];return <div key={k} className="lesson-row" style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) auto',gap:12,padding:'10px 0'}}><span>{m.id==='01'?<a className="lesson-link" href={`/merl/foundations/01/${i+1}`}><b>{m.id}.{i+1} {lesson}</b></a>:<b>{m.id}.{i+1} {lesson}</b>}<br/><small>Complete the guided practice, then record the evidence gate.</small></span><button type="button" disabled={done||!!busy} onClick={()=>run(k,()=>markLessonEvidence(m.id,i))}>{done?'Recorded ✓':busy===k?'Saving…':'Record lesson evidence'}</button></div>})}
