...
export default function MerlFoundations(){
 ...
 return <main id="main-content" className="merl-main" aria-busy={loading||!!busy} style={{maxWidth:1100,margin:'0 auto',padding:'40px 24px',fontFamily:'system-ui',color:'#0b2c4d'}}>
  <a className="skip-link" href="#module-list">Skip to course modules</a>
  ...
  {loading&&<p className="sr-only" role="status" aria-live="polite">Loading your course progress.</p>}
  ...
  <section className="progress-card" aria-labelledby="credential-progress-label" style={{padding:20,border:'1px solid #d7dee7',borderRadius:12,margin:'28px 0',background:'#f6f8fb'}}><b id="credential-progress-label">Credential progress {loading?'…':`${credentialProgress}%`}</b><span className="progress-count" aria-live="polite" style={{float:'right'}}>{achieved}/25 evidence gates</span><div role="progressbar" aria-labelledby="credential-progress-label" aria-valuemin={0} aria-valuemax={100} aria-valuenow={loading?undefined:credentialProgress} style={{height:8,background:'#e5e7eb',borderRadius:8,marginTop:12}}><div style={{height:8,width:`${credentialProgress}%`,background:'#1565c0',borderRadius:8}}/></div></section>
  <section id="module-list" aria-label="MERL Level 1 modules" style={{display:'grid',gap:14}}>{modules.map((m,index)=>{const locked=index>0&&!complete(modules[index-1].id),expanded=open===m.id,mp=p(m.id),isComplete=complete(m.id);return <article key={m.id} style={{border:'1px solid #d7dee7',borderRadius:12,overflow:'hidden',opacity:locked?.58:1}}>
   <button type="button" className="module-toggle" disabled={locked||loading} aria-expanded={!locked&&expanded} aria-controls={`module-panel-${m.id}`} onClick={()=>setOpen(expanded?'':m.id)} style={{width:'100%',border:0,background:isComplete?'#eef8f1':'white',padding:20,display:'grid',gridTemplateColumns:'60px minmax(0,1fr) auto',gap:16,alignItems:'center',textAlign:'left',color:'#0b2c4d'}}><b style={{fontSize:24}}>{m.id}</b><div><h2 style={{margin:0,fontSize:20}}>{m.title}</h2><p style={{margin:'6px 0 0'}}>Professional output: {m.output}</p></div><b>{locked?'LOCKED':isComplete?'COMPETENT ✓':expanded?'CLOSE':'OPEN MODULE'}</b></button>
   {expanded&&!locked&&<div id={`module-panel-${m.id}`} className="module-panel" style={{padding:'0 24px 24px'}}><hr style={{border:0,borderTop:'1px solid #e5e7eb'}}/><h3>Learning sequence</h3>{m.lessons.map((lesson,i)=>{const k=`${m.id}-${i}`,done=!!mp?.lesson_evidence?.[k];return <div key={k} className="lesson-row" style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) auto',gap:12,padding:'10px 0'}}><span>{['01','02','03','04','05'].includes(m.id)?<a className="lesson-link" href={`/merl/foundations/${m.id}/${i+1}`}><b>{m.id}.{i+1} {lesson}</b></a>:<b>{m.id}.{i+1} {lesson}</b>}<br/><small>Complete the guided practice, then record the evidence gate.</small></span><button type="button" disabled={done||!!busy} onClick={()=>run(k,()=>markLessonEvidence(m.id,i))}>{done?'Recorded ✓':busy===k?'Saving…':'Record lesson evidence'}</button></div>})}
    <div className="content-card" style={{marginTop:18,padding:16,border:'1px solid #d7dee7',borderRadius:10}}><label htmlFor={`evidence-${m.id}`}><b>Professional evidence submission</b></label><p>Submit the actual {m.output.toLowerCase()}. Minimum 20 characters; the submission is stored with your learner identity and timestamp.</p><textarea id={`evidence-${m.id}`} rows={5} value={evidence[m.id]||''} onChange={e=>setEvidence(v=>({...v,[m.id]:e.target.value}))} disabled={!!mp?.evidence_submission} style={{width:'100%',boxSizing:'border-box',padding:10}}/><button type="button" disabled={!lessonsDone(m)||!!mp?.evidence_submission||!!busy} onClick={()=>run(`ev-${m.id}`,()=>submitProfessionalEvidence(m.id,m.output,evidence[m.id]||''))}>{mp?.evidence_submission?'Submitted ✓':busy===`ev-${m.id}`?'Submitting…':'Submit professional evidence'}</button></div>
    <div className="content-card" style={{marginTop:12,padding:16,border:'1px solid #d7dee7',borderRadius:10}}><b>Scored module quiz</b><p>Pass mark 70%; maximum two attempts. Answers are graded server-side and the protected answer key is never sent to the browser.</p>{mp?.quiz_score!=null&&<p><b>Best score: {mp.quiz_score}% · Attempts: {mp.quiz_attempts}/2 {mp.quiz_passed?'· PASSED':''}</b></p>}
     {!mp?.quiz_passed&&mp?.evidence_submission&&!quiz[m.id]&&<button type="button" disabled={!!busy||mp.quiz_attempts>=2} onClick={()=>loadQuiz(m.id)}>Load quiz</button>}
     ...
     {quiz[m.id]?.length>0&&!mp?.quiz_passed&&<button type="button" disabled={!!busy||quiz[m.id].some(q=>answers[m.id]?.[q.question_id]===undefined)} onClick={()=>run(`grade-${m.id}`,async()=>{const result=await gradeModuleQuiz(m.id,answers[m.id]||{});if(!result.ok) throw new Error(result.error);setMessage(`Quiz score: ${result.score}% · Attempt ${result.attempts}/2`);setQuiz(v=>({...v,[m.id]:[]}));setAnswers(v=>({...v,[m.id]:{}}))})}>Submit quiz for scoring</button>}
  ...
  <style jsx global>{`
   ...
   @media (max-width:640px){
    ...
    .merl-title{font-size:32px!important;line-height:1.15}
    .progress-count{display:block;float:none!important;margin-top:8px}
    .module-toggle{grid-template-columns:42px minmax(0,1fr)!important;gap:10px!important;padding:16px!important}
    .module-toggle>strong,.module-toggle>b:last-child{grid-column:1/-1}
    .module-panel{padding:0 16px 20px!important}
    .lesson-row{grid-template-columns:1fr!important}
    .lesson-row button{width:100%}
    .content-card,.case-card,.credential-card{padding:16px!important}
    .content-card button{width:100%;margin-top:8px}
    .case-links{flex-direction:column}
    .case-links a{display:flex;align-items:center;padding:4px 0}
   }
