...
export default async function Level2Portfolio(){
 ...
 return <main className="portfolio"><nav><a href="/merl/applied">← Level 2 modules</a></nav><header><p>MEAL PROFESSIONAL PATHWAY · LEVEL 2</p><h1>Applied MEAL competency portfolio</h1><span>System completion verifies the evidence trail; professional certification still requires human judgement.</span></header>
  ...
  <section className="standard"><h2>Professional MEAL review rubric</h2><p>Human reviewers score each dimension 0–3. Automated completion establishes readiness for review; it does not independently confer professional competence.</p><ol>{MEAL_RUBRIC.map(item=><li key={item.id}><b>{item.label}{item.critical?' · CRITICAL GATE':''}:</b> {item.description}</li>)}</ol><p><b>Competent:</b> at least 16/24 and no zero in integrity/protection or accountability. <b>Strong professional practice:</b> at least 20/24 with no dimension below 2.</p></section><section className="standard"><h2>Accountability, learning and adaptation evidence</h2><ul><li>Stakeholder and information-needs map</li><li>Feedback/accountability mechanism and response/closure protocol</li><li>Privacy-safe feedback register or synthetic equivalent</li><li>Learning agenda and pause-and-reflect record</li><li>Adaptation/decision log with evidence, owner, due date and follow-up</li><li>Communication-back, ethics, safeguarding and data-protection note</li></ul></section>
  {ready?<PortfolioControls learnerName={String(user.user_metadata?.full_name || user.user_metadata?.name || '')} review={review}/>:<p className="notice"><b>Portfolio submission remains locked until all 25 competency gates pass.</b></p>}
  <style>{`
