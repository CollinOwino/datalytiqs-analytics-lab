...
export default async function MerlApplied(){
 ...
 return <main className="applied-shell">
  ...
  <section id="level-two-modules" aria-labelledby="modules-heading"><h2 id="modules-heading">Learning sequence</h2><div className="module-grid">{modules.map((module,index)=><article key={module.id} aria-labelledby={`module-${module.id}`}><div><b>{module.id}</b><span>{complete?'AVAILABLE':'LOCKED'}</span></div><h3 id={`module-${module.id}`}>{module.title}</h3><p>Professional output: <b>{module.output}</b></p><ol>{module.lessons.map((lesson,lessonIndex)=><li key={lesson}>{complete?<a href={`/merl/applied/${module.id}/${lessonIndex+1}`}>{module.id}.{lessonIndex+1} {lesson}</a>:<span>{module.id}.{lessonIndex+1} {lesson}</span>}</li>)}</ol></article>)}</div></section>
  <aside className="build-note" role="note"><b>Progression notice:</b> Level 2 content is complete across Modules 06–10, including professional templates. Persistent evidence, assessment and portfolio gates are the final implementation phase before enrolment readiness.</aside>
