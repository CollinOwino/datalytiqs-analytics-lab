...
export default async function MerlApplied(){
 ...
 return <main className="applied-shell">
  ...
  <section id="level-two-modules" aria-labelledby="modules-heading"><h2 id="modules-heading">Learning sequence</h2><div className="module-grid">{modules.map((module,index)=><article key={module.id} aria-labelledby={`module-${module.id}`}><div><b>{module.id}</b><span>{index===0&&complete?'AVAILABLE':'PLANNED'}</span></div><h3 id={`module-${module.id}`}>{module.title}</h3><p>Professional output: <b>{module.output}</b></p><ol>{module.lessons.map((lesson,lessonIndex)=><li key={lesson}>{index===0&&complete?<a href={`/merl/applied/06/${lessonIndex+1}`}>{module.id}.{lessonIndex+1} {lesson}</a>:<span>{module.id}.{lessonIndex+1} {lesson}</span>}</li>)}</ol></article>)}</div></section>
  <aside className="build-note" role="note"><b>Progression notice:</b> All three Module 06 lessons and the professional template are available. Persistent Level 2 evidence and assessment gates will be enabled only after their database rules and acceptance tests are deployed.</aside>
