...
export default async function ModuleSixLesson({params}:{params:Promise<{lesson:string}>}){
 const {lesson}=await params
 if(!['1','2','3'].includes(lesson)) notFound()
 const supabase=await createClient()
 const {data:{user}}=await supabase.auth.getUser()
 if(!user) redirect(`/login?next=${encodeURIComponent(`/merl/applied/06/${lesson}`)}`)
