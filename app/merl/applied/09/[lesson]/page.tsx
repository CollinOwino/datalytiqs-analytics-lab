...
export default async function ModuleNineLesson({params}:{params:Promise<{lesson:string}>}){
 const {lesson}=await params
 if(!['1','2','3'].includes(lesson)) notFound()
 const supabase=await createClient()
 const {data:{user}}=await supabase.auth.getUser()
 if(!user) redirect(`/login?next=${encodeURIComponent(`/merl/applied/09/${lesson}`)}`)
