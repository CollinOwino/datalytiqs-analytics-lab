...
export async function updateCertificateSignatory(formData: FormData) {
  ...
  const signatureImageUrl =
    String(formData.get("signature_image_url") || "").trim() || null;
  if (!id || displayName.length < 2 || title.length < 2)
    throw new Error("Complete the signatory name and title.");
...
export async function issueMerlFoundationsCredential(formData: FormData) {
  ...
  const { data, error } = await supabase.rpc(
    "issue_merl_foundations_credential",
    {
      p_user_id: userId,
      p_learner_name: learnerName,
...
export async function revokeCredential(formData: FormData) {
  ...
  if (!credentialId || reason.length < 10)
    throw new Error(
      "Provide the credential ID and a substantive revocation reason.",
    );
  ...
  revalidatePath(`/certificate/${credentialId}`);
}
