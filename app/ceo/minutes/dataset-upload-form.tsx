...
export default function DatasetUploadForm() {
  ...
  return <form action={action} className="minutes-dataset-form" aria-busy={pending}>
    ...
    {state.message && <div role="status" aria-live="polite" className={`minutes-feedback minutes-feedback-${state.ok ? 'success' : 'error'}`}>{state.message}{!state.ok && !state.message.includes('Do not re-upload') && ' Re-select the file before retrying.'}</div>}
