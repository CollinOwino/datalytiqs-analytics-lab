...
export async function processMinutes(formData:FormData){
 ...
 const title=clean(formData.get('title'));const text=clean(formData.get('minutes_text'))
 if(title.length<3||text.length<100)throw new Error('Provide a title and at least 100 characters of meeting minutes.')
 const parsed=parseMinutes(text)
 const{data:doc,error:de}=await supabase.from('ceo_documents').insert({organization_id:organizationId,uploaded_by:user.id,title,document_type:'minutes',source_text:text,processing_status:'ready',metadata:{ingestion:'text',parser:'deterministic-v1'}}).select('id').single();if(de)throw new Error(de.message)
 ...
 revalidatePath('/ceo/minutes');return{documentId:doc.id,meetingId:meeting.id,jobId:job.id}
}
