...
export const topicContent: Record<string, TopicContent> = {
  ...
  '2.0': {
    ...
    quiz: [
      { code: 'q1', question: 'A school wants to improve fee-support decisions but has not defined a success measure. Which CRISP phase must resolve this first?', options: [['a','Data preparation'],['b','Business understanding'],['c','Deployment'],['d','Modelling']] },
      { code: 'q2', question: 'A payment dataset design now specifies PostgreSQL tables, column types and indexes. Which model layer is this?', options: [['a','Conceptual'],['b','Logical'],['c','Physical'],['d','Narrative']] },
      { code: 'q3', question: 'An audit team has 40 hours and must choose which high-risk exceptions to review. Which analysis recommends an allocation under that limit?', options: [['a','Descriptive'],['b','Diagnostic'],['c','Predictive'],['d','Prescriptive']] },
      { code: 'q4', question: 'You need to show Organic conversion rates from January through April without hiding the direction of change. Which chart best displays the trend?', options: [['a','Line chart'],['b','Pie chart'],['c','Radar chart'],['d','Single KPI card']] },
      { code: 'q5', question: 'The customer-risk dataset has 780,000 rows, daily refresh and eight users. What is the strongest basis for choosing its analytical tool?', options: [['a','Popularity alone'],['b','Highest complexity'],['c','Fit to scale, repeatability, governance and users'],['d','Number of visual effects']] },
    ],
  ...
  '3.0': {
    ...
    quiz: [
      { code: 'q1', question: 'To compare 2025 cost of sales with 2023, you convert each income-statement line into a percentage of which base?', options: [['a','Revenue'],['b','Total assets'],['c','Closing cash'],['d','Share capital']] },
      { code: 'q2', question: 'Receivables grew much faster than revenue from 2023 to 2025. What risk should the analyst investigate first?', options: [['a','Faster cash conversion'],['b','Collection and liquidity pressure'],['c','Lower credit exposure'],['d','Automatic profitability growth']] },
      { code: 'q3', question: 'The investment file shows a KES 4.5m outflow in year 0. Where should this cash flow appear in the NPV model?', options: [['a','At time zero'],['b','After the final year'],['c','Inside the discount rate'],['d','Only in a chart']] },
      { code: 'q4', question: 'The base investment NPV is positive at a 12% discount rate. What conclusion is supported under the stated cash-flow assumptions?', options: [['a','Guaranteed cash'],['b','Value above the required return under stated assumptions'],['c','Zero risk'],['d','Immediate payback']] },
      { code: 'q5', question: 'Management wants a downside case with both lower volume and higher costs. How does this differ from changing only one driver?', options: [['a','It changes coherent groups of assumptions'],['b','It removes assumptions'],['c','It never uses formulas'],['d','It only restates history']] },
    ],
  ...
  '4.0': {
    ...
    quiz: [
      { code: 'q1', question: 'The factory produced 8,600 units against 8,000 planned. Why flex variable costs before comparing actual spending?', options: [['a','It adjusts allowed variable cost to actual activity'],['b','It hides volume'],['c','It removes fixed costs'],['d','It guarantees favourable variances']] },
      { code: 'q2', question: 'Alpha Supplies invoice AS-1042 appears in T001 and T003. What does that duplicate flag establish by itself?', options: [['a','Fraud conclusively'],['b','An exception requiring investigation'],['c','A valid payment'],['d','A tax liability']] },
      { code: 'q3', question: 'T002 invoice is KES 15,000 above the recorded receipt. Which three documents should the auditor reconcile?', options: [['a','Order, receipt and invoice'],['b','Budget, chart and memo'],['c','Payroll, tax rate and loan'],['d','Forecast, policy and email']] },
      { code: 'q4', question: 'A county revised its annual budget before the reporting period closed. Which denominator supports execution analysis for that period?', options: [['a','Revised budget for the same period'],['b','Any prior figure'],['c','Closing cash only'],['d','Number of employees']] },
      { code: 'q5', question: 'The tax practice file lists vehicle depreciation and tax adjustments separately. Why keep dated rates and adjustments apart from source amounts?', options: [['a','To improve transparency and auditability'],['b','To avoid reconciliation'],['c','To remove source documents'],['d','To conceal rates']] },
    ],
  ...
  '5.0': {
    ...
    quiz: [
      { code: 'q1', question: 'Managers bypass a new KPI pack because its definitions are unclear. What combination is most likely to improve sustained use?', options: [['a','Technology alone'],['b','Usefulness, trust, capability and workflow fit'],['c','Mandatory colour schemes'],['d','More metrics']] },
      { code: 'q2', question: 'A regional dashboard needs only aggregated counts but its source includes national IDs. What does data minimisation require?', options: [['a','Collect every available field'],['b','Collect only data necessary for the defined purpose'],['c','Retain data forever'],['d','Publish raw identifiers']] },
      { code: 'q3', question: 'A learner-support model flags a person for a consequential intervention. What safeguard is essential before action?', options: [['a','No human involvement'],['b','Human oversight and a challenge route'],['c','Hidden criteria'],['d','Unlimited reuse']] },
      { code: 'q4', question: 'A daily, sensitive customer-risk model has 780,000 rows and eight users. When is a governed platform justified?', options: [['a','When scale, concurrency or control requirements exceed them'],['b','Whenever a chart is needed'],['c','Only after total failure'],['d','Never']] },
      { code: 'q5', question: 'Shared credentials are identified as a high-risk control failure. What must the security programme provide beyond prevention?', options: [['a','Detection, response and recovery'],['b','A larger file'],['c','Fewer owners'],['d','Hidden documentation']] },
    ],
