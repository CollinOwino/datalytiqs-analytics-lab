...
export default async function ExamHubPage({ searchParams }: { searchParams: Promise<{ state?: string }> }) {
  ...
  const firstAccessibleTopic = nextTopic || topics.find(topic => topic.sequence_no <= programme.free_topic_limit || hasFullAccess || grants.some(grant => grant.access_level === 'topic' && grant.topic_id === topic.id && grantIsActive(grant)))
  ...
  return <main className="exam-hub">
    ...
    <section className="exam-hero" id="overview">
      ...
      <aside aria-label="Programme facts"><div><b>5</b><span>syllabus topics</span></div><div><b>15</b><span>guided subtopics</span></div><div><b>3</b><span>topics free</span></div><div><b>20</b><span>practice cases</span></div></aside>
    </section>
    ...
    <section className="exam-status" aria-label="Learner status">
      ...
      <article><span>PROGRESS</span><b>{enrollment ? `${progressPercent}%` : '—'}</b><small>{enrollment ? `${completedSubtopics}/${totalSubtopics} subtopics · ${completed}/${topics.length} topics` : 'Sign in and enrol to track progress'}</small></article>
    ...
    {user&&enrollment&&<section className="learner-command" aria-label="Study command centre"><div><span className="exam-kicker">YOUR NEXT BEST ACTION</span><h2>{nextTopic?`Continue ${nextTopic.code} · ${nextTopic.title}`:'Accessible topics complete'}</h2><p>{nextTopic?'Resume the next accessible topic, complete its evidence gates and then attempt the topic assessment.':'Review your topic evidence and practical feedback. The full online mock is still in preparation.'}</p>{nextTopic?<a className="exam-button primary" href={`/exam-hub/${nextTopic.code}`}>Resume learning</a>:<a className="exam-button primary" href="#assessments">Review assessments</a>}</div><aside><span>OVERALL COVERAGE</span><b>{progressPercent}%</b><small>{completedSubtopics}/{totalSubtopics} subtopics complete</small>{targetDays!==null?<><span>EXAM COUNTDOWN</span><b>{targetDays}</b><small>days to target examination date</small></>:null}</aside></section>}
    ...
    <section className="assessment-band" id="assessments" aria-labelledby="assessment-title"><div className="exam-section-head inverse"><div><span className="exam-kicker">ASSESSMENT CENTRE</span><h2 id="assessment-title">Practise, submit and receive feedback</h2></div><p>Each accessible topic contains case questions, a scored quiz and an assessor-reviewed practical. The full mock is currently a planning blueprint.</p></div><div className="assessment-grid"><article><span>TOPIC PRACTICE</span><h3>Case question bank</h3><p>Work through 20 original self-check cases across five topics, then take a five-question scored quiz for each topic.</p><b>20 cases · 25 scored items</b><a href={`/exam-hub/${firstAccessibleTopic?.code || '1.0'}#practice-bank-title`}>Open case practice →</a></article><article><span>PRACTICAL WORK</span><h3>Spreadsheet competency tasks</h3><p>Prepare models, analyses and decision outputs against a clear assignment checklist and 20-mark rubric.</p><b>Assessor reviewed</b><a href={`/exam-hub/${firstAccessibleTopic?.code || '1.0'}#practical-submission`}>Open an assignment →</a></article>{(assessments || []).map((assessment: any) => <article key={assessment.id}><span>FULL MOCK BLUEPRINT</span><h3>{assessment.title}</h3><p>{assessment.time_limit_minutes} minutes · {assessment.metadata?.total_marks || 100} marks · proposed pass mark {assessment.pass_mark}%. Online attempts are not yet available.</p><b>Planning reference</b><a href="#syllabus">Study by topic →</a></article>)}</div></section>
