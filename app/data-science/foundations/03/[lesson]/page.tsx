import {notFound,redirect} from 'next/navigation'
import {createClient} from '../../../../../lib/supabase/server'
import ModuleThreeLesson from './lesson-experience'

export default async function Page({params}:{params:Promise<{lesson:string}>}){
 const {lesson}=await params;if(!['1','2','3'].includes(lesson))notFound()
 const supabase=await createClient();const{data:{user}}=await supabase.auth.getUser()
 if(!user)redirect(`/login?next=${encodeURIComponent(`/data-science/foundations/03/${lesson}`)}`)
 const {data,error}=await supabase.rpc('get_data_science_module3_state');const state=Array.isArray(data)?data[0]:data
 if(error||!state?.module_02_completed)redirect('/data-science/foundations/02')
 return <ModuleThreeLesson lesson={lesson as '1'|'2'|'3'}/>
}
