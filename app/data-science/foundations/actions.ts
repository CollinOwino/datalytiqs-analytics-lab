...
export async function getDataScienceModuleTenState(){const{supabase}=await authenticated();const{data,error}=await supabase.rpc('get_data_science_module10_state');if(error)throw new Error(`Unable to load Module 10 progress: ${error.message}`);return row(data)}
export async function markDataScienceModuleTenLesson(lessonIndex:number){if(!Number.isInteger(lessonIndex)||lessonIndex<0||lessonIndex>2)throw new Error('Invalid lesson record.');const{supabase}=await authenticated();const{error}=await supabase.rpc('mark_data_science_module10_lesson',{p_lesson_index:lessonIndex});if(error)throw new Error(error.message);revalidatePath('/data-science/foundations/10');return{ok:true}}
export async function submitDataScienceModuleTenEvidence(content:string){const clean=content.trim();if(clean.length<100||clean.length>20000)throw new Error('Professional evidence must contain 100 to 20,000 characters.');const{supabase}=await authenticated();const{data,error}=await supabase.rpc('submit_data_science_module10_evidence',{p_evidence_type:'Data Science Capstone Dossier and Portfolio Defence',p_content:clean});if(error)throw new Error(error.message);revalidatePath('/data-science/foundations/10');return{ok:true,id:data}}
export async function getDataScienceModuleTenQuiz(){const{supabase}=await authenticated();const{data,error}=await supabase.rpc('get_data_science_module10_quiz');if(error)throw new Error(error.message);return data??[]}
export async function gradeDataScienceModuleTenQuiz(answers:Record<string,string>){const{supabase}=await authenticated();try{const{data,error}=await supabase.rpc('grade_data_science_module10_quiz',{p_answers:answers});if(error)return{ok:false as const,error:error.message};const result=row(data)as{score:number;passed:boolean;attempts:number;programme_completed:boolean}|null;if(!result)return{ok:false as const,error:'Quiz grading returned no result; no attempt was recorded.'};const{data:persisted,error:verifyError}=await supabase.rpc('get_data_science_module10_state');if(verifyError)return{ok:false as const,error:`The quiz was graded, but persistence could not be verified: ${verifyError.message}`};const state=row(persisted)as{quiz_attempts:number;quiz_score:number|null;programme_completed:boolean}|null;if(!state||Number(state.quiz_attempts)<Number(result.attempts)||state.quiz_score===null||Boolean(state.programme_completed)!==Boolean(result.programme_completed))return{ok:false as const,error:'The quiz transaction did not persist its score, attempt count and programme status.'};revalidatePath('/data-science/foundations/10');return{ok:true as const,score:Number(result.score),passed:Boolean(result.passed),attempts:Number(result.attempts),programmeCompleted:Boolean(result.programme_completed)}}catch(error){return{ok:false as const,error:error instanceof Error?error.message:'Quiz grading failed.'}}}
