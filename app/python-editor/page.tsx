...
type ExecutionOutput={id:string;kind:string;sequence:number;text?:string;table?:{columns:string[];rows:(string|number|boolean|null)[][]};chart?:{format:string;data?:string;url?:string;title?:string}}
...
export default function PythonEditor(){
 ...
 async function run(){
  setRunning(true);setExecutionError('');setOutputs([]);setExecutionStatus('Submitting');setTab('console')
  try{
   const submit=await fetch('/api/executions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({requestId:crypto.randomUUID(),caseId:'001',language:'python',code,metadata:{projectId:project,stage:'03'},limits:{timeoutMs:30000,memoryMb:512,cpuSeconds:20,maxOutputBytes:2000000,network:'disabled'}})})
   const accepted=await submit.json()
   if(!submit.ok)throw new Error(accepted?.error?.message||'Unable to submit execution.')
   const executionId=accepted.executionId as string
   setExecutionStatus(accepted.status||'queued')
   for(let i=0;i<20;i++){
    await new Promise(r=>setTimeout(r,500))
    const res=await fetch(`/api/executions/${encodeURIComponent(executionId)}`,{cache:'no-store'})
    const result=await res.json() as ExecutionResult
    if(!res.ok)throw new Error(result?.error?.message||'Unable to read execution result.')
    setExecutionStatus(result.status)
    if(result.outputs)setOutputs(result.outputs.sort((a,b)=>a.sequence-b.sequence))
    if(['succeeded','failed','timed_out','cancelled'].includes(result.status)){
      if(result.status!=='succeeded')setExecutionError(result.error?.message||`Execution ${result.status}.`)
      return
    }
   }
   throw new Error('Execution did not finish within the polling window.')
  }catch(e){setExecutionStatus('failed');setExecutionError(e instanceof Error?e.message:'Execution failed.')}finally{setRunning(false)}
 }

 const stdout=outputs.filter(o=>o.kind==='stdout'||o.kind==='stderr'||o.kind==='error').map(o=>o.text||'').filter(Boolean).join('\n')
 const tableOutput=outputs.find(o=>o.kind==='table'&&o.table)?.table
 const chartOutput=outputs.find(o=>o.kind==='chart'&&o.chart)?.chart
 ...
 <section className="py-context"><div><span className="eyebrow gold">STAGE 03 · CONDUCT DESCRIPTIVE ANALYSIS</span><h2>Describe the evidence before explaining it.</h2><p>The Run Analysis control is now connected to the Analytics Lab execution API boundary. The current provider may still be a contract-development mock until the external sandbox is configured.</p></div><a href={project?`/data-explorer?project=${encodeURIComponent(project)}`:'/data-explorer'}>Open Data Explorer →</a></section>
